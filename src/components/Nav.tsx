  ...
  return (
    <header className="sticky top-0 z-40 pt-3 sm:pt-4">
      <div className="container-page">
        <nav className="flex h-16 items-center justify-between gap-4 rounded-full border border-white/10 bg-black/85 pl-6 pr-3 text-white shadow-xl backdrop-blur-md">
          {/* Logo */}
          <Link
            href="/"
          ...
          >
            <AnybugLogo className="h-8 w-auto text-white sm:h-9" />
          </Link>
        ...
        <div
          className={`overflow-hidden md:hidden ${
            open ? "mt-2 max-h-80" : "max-h-0"
          } transition-[max-height] duration-300`}
        >
          <ul className="flex flex-col gap-1 rounded-3xl border border-white/10 bg-black/90 p-3 text-white shadow-xl backdrop-blur-md">
            {site.nav.map((item) => (
              <li key={item.href}>
                <Link
                  href={item.href}
                  onClick={() => setOpen(false)}
                  className="block rounded-2xl px-4 py-3 text-base font-medium uppercase tracking-wide text-white/90 transition hover:bg-white/5 hover:text-brand"
                >
                  {item.label}
                </Link>
              </li>
